...
def load_navier_stokes_pt(data_path,
                          train_resolution,
                          n_train,
                          n_tests,
                          batch_size,
    ...
    data = torch.load(
        Path(data_path)
        .joinpath('nsforcing_' + train_resolution_str + '_train.pt')
        .as_posix())
...
def _load_navier_stokes_test_HR(data_path,
    ...
    x_test = data['x'][:n_test, ::downsample_factor, ::downsample_factor]\
        .unsqueeze(channel_dim)\
        .clone()
    y_test = data['y'][:n_test, ::downsample_factor, ::downsample_factor]\
        .unsqueeze(channel_dim)\
        .clone()
    del data
